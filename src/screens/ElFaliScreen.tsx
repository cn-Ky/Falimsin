...
type Nav = NativeStackNavigationProp<FalStackParamList, 'El'>;

export function ElFaliScreen() {
  const navigation = useNavigation<Nav>();
  const { session } = useAuth();
  const { submit, submitting } = useImageFalSubmit(session?.user.id, 'el');
  ...
  return (
    <ScreenGradient>
      <ScrollView contentContainerStyle={styles.content}>
        <SectionTitle
          title="El Falı 🖐️"
          subtitle="Avuç içinin çizgilerini net gösteren, iyi ışıklı bir fotoğraf yükle"
        />
        ...
        <AppTextInput
          ...
          placeholder="Örn: Kariyerim hakkında ne görüyorsun?"
          value={question}
        ...
        {submitting ? (
          <View style={styles.loadingRow}>
            ...
            <Text style={styles.loadingText}>Çizgiler okunuyor…</Text>
          </View>
        ) : (
